...
LOCK_FILE="/run/scratch-cleanup.lock"
...
The file ${file} has now been automatically deleted due to it not being accessed or modified in the last ${DAYS_TO_KEEP} days.
...
Any file in any other folder in the "/scratch/" directory will be deleted after ${DAYS_TO_KEEP} days of it being unaccessed or unmodified.
...
After ${DAYS_TO_KEEP} days without being accessed or modified files in "/scratch/" sub-directories except the ones in "/scratch/datasets" will be automatically deleted.
...
EOF
    ...
    done < <(find "$scratch_dir" -type f \( -atime +$DAYS_TO_NOTIFY -a -mtime +$DAYS_TO_NOTIFY \) -print0 2>/dev/null)
}
